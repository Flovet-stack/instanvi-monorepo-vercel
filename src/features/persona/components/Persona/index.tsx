...
export default function PersonaItem({
  ...
  const deletePersona = useMutation(((uuid: string) =>
    PersonasAPI.deletePersonas(uuid)) as any);
  ...
  return (
    <div
      key={persona?.uuid}
      className="border rounded p-5 group hover:border-green-500"
    >
      <div className="flex">
        <div className="flex">
          <Avatar
      ...
      <div>
        <div className="flex hidden group-hover:flex gap-3 mt-4 items-center justify-end">
          <EditIcon
            onClick={() => navigate.push(`persona/edit/${persona?.uuid}`)}
          />
