...
import { usePersona } from "@/api/persona/usePersona";
...
export default function Persona() {
    ...
    const { data: persona, isLoading } = usePersona(id as string ?? '6b53970a-a08f-4e00-a85e-970b52e6c923')

    const ContextData = useContext(CurrentUserContext as any) as any
    const editPersona = useMutation(((data: any) => PersonasAPI.updatePersonas(id as string, data)) as any)
    ...
    const [personaUUID, setPersonaUUID] = useState<any>('')

    console.log("LOG", id);

    useEffect(() => {
            const personaData = persona?.data?.data
            ContextData.setUsern(personaData?.name)
            ContextData.setMarital(personaData?.social_status)
            // ContextData.setLanguage(personaData?.language)
            // // setAudience(persona?.data?.data?.)
            ContextData.setSex(personaData?.sex)
            ContextData.setAge(personaData?.age)
            ContextData.setIncome(personaData?.income_level)
            ContextData.setInterest(personaData?.interests)
            ContextData.setIndustry(personaData?.ethnicity)
            ContextData.setDevice(personaData?.device_type)
            ContextData.setOperatingSystem(personaData?.operating_system)
            ContextData.setOffLimit(personaData?.off_limit)
            ContextData.setCarrier(personaData?.carrier_isp)
    }, [isLoading])
    ...
    const save = async (e: any) => {
        ...
        editPersona.mutate(data, {
            onSuccess: () => {
                toast.success(`Persona ${ContextData.usern} edited Succesfully`);
                router.push('/persona')
                setAnimation(false);
