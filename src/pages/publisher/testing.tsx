...
const Audience: FC = () => {
  ...
  const [social] = useState<SocialStatus[]>([
    {
      id: 1,
      name: 'Single',
      checked: false,
  ...
  const handleSocials = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    localStorage.setItem(
      'channel',
      JSON.stringify({ ...storage, marital_status: newArray })
    );
  };
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        audience_description: {
          ...storage.audience_description,
          marital_status: updatedPeople,
        },
      })
    );
              ...
              {people.map((person, index) => (
                <div className="flex justify-between mt-4" key={index}>
                  ...
                  <input
                    ...
                    value={
                      updatedPeople.find((p) => p.status === person.status)
                        ?.total || ''
                    }
